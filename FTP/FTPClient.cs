...
namespace Hani.FTP
{
    internal sealed partial class FTPClient : NetworkClient
    {
        ...
        private const int noopAfter = 590;
        ...
        internal override async Task<bool> ConnectAsync(bool reconnect = false, bool tryReconnect = true)
        {
            ...
            if (ControlSocket != null)
            {
                ...
                if (secureConnection) ControlStream = await SslClient.ConnectAsync(ControlStream, Host);
            ...
            if (IsConnected && (Encryption.Type == EncryptionType.ExplicitType))
            {
                ...
                {
                    ControlStream = await SslClient.ConnectAsync(ControlStream, Host);
        ...
        {
            if (secureConnection) DataStream = await SslClient.ConnectAsync(DataStream, Host);
        ...
        private async Task<bool> _setHomeDirectoryAsync()
        {
            ...
            {
                InfoMessage("NO directory been set for " + UserName, MessageType.Warning);
                return false;
            }
            return true;
        }
